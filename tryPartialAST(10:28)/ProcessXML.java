...
public class ProcessXML {
    ...
    public static void main(String[] args) {
                ...
                && rightBracketPos < nextLeftBracketPos)
        {
            ...
            String tagName = inputString.substring(start+1, rightBracketPos);
            
            if(inputString.charAt(start+1) != '/') //遇到一个STag
            {
            	switch(tagName){ //把不想在AST中出现的STag的'<'变为'#'
                	case "specifier": outputString += "#" + tagName + ">";
                                      break;
                	    	 default: outputString += "<" + tagName + ">";
              }
            }
            else	
